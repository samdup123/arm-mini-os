...
//***IMPORTANT - change bcm2835_bsc0 to bcm2835_bsc1 or vice-versa depending on
//version of model B PI used.

// The delay time in uS needed to transmit one byte for 100KHZ SCL (see
// formula).
...
void bcm2835_i2c_read(uint8_t sradd, char* buff) {
  ...
  while (!(bcm2835_peri_read_nb(status) & BCM2835_BSC_S_TA)) {
    ...
    if (bcm2835_peri_read(status) & BCM2835_BSC_S_DONE) break;
  }
